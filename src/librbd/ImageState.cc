...
public:
  ...
  void shut_down(Context *on_finish) {
    ...
    ldout(m_cct, 20) << __func__ << ": completing shut down" << dendl;
  ...
  void unregister_watcher(uint64_t handle, Context *on_finish) {
    ldout(m_cct, 20) << __func__ << ": handle=" << handle << dendl;
    ...
    if (on_finish) {
      ldout(m_cct, 20) << __func__ << ": completing unregister" << dendl;
  ...
  void send_notify(uint64_t handle, UpdateWatchCtx *watcher) {
    ...
    ldout(m_cct, 20) << __func__ << ": handle=" << handle << ", watcher="
		     << watcher << dendl;
...
ImageState<I>::~ImageState() {
  ...
  assert(m_update_watchers == nullptr);
}
...
void ImageState<I>::handle_open(int r) {
  ...
  if (r == 0) {
    assert(m_update_watchers == nullptr);
    m_update_watchers = new ImageUpdateWatchers(cct);
  }
